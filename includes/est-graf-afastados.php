...
<?php getTitulo('GRÁFICOS DE AFASTADOS (' . date('d/m/Y', strtotime("-1 year")) . ' A ' .date('d/m/Y', strtotime("today")). ')'); ?>
...
try{
	$rsTotal = $conx->prepare('
								SELECT COUNT(*) as qtd FROM fj_afast_has_fj_jovem a
								INNER JOIN fj_jovem b ON (a.fk_jovem_id = b.id)
								WHERE a.data_afastamento > DATE_ADD(NOW(), INTERVAL -1 YEAR)
								');
	$rsTotal->execute();
...
try{
	$rs = $conx->prepare('
						SELECT COUNT(*) as qtd, b.nome_motivo FROM fj_afast_has_fj_jovem a
						INNER JOIN fj_motiv_afastado b ON (a.fk_motiv_afastado_id = b.id)
						INNER JOIN fj_jovem c ON (a.fk_jovem_id = c.id)
						WHERE a.data_afastamento > DATE_ADD(NOW(), INTERVAL -1 YEAR)
						GROUP BY a.fk_motiv_afastado_id
						');
	$rs->execute();
...
<div class="grafico-pagina">
	...
	for ($i = 0; $i <= count($rs) - 1; $i++){
		echo '<div class="barra" style="left:' . $leftBarra . 'px; width:' . $larguraBarra . 'px;" data-height="' . ($rs[$i]['qtd'] / $rsTotal) * 100 . '"><div>' . number_format(($rs[$i]['qtd'] / $rsTotal) * 100, 1, ',', '') . '%</div><div class="descricao">' . $rs[$i]['nome_motivo'] . '</div></div>';
...
<div class="grafico-pagina">
	...
	<?php
	for ($i = 0; $i <= count($rs) - 1; $i++){
		echo '<div class="barra" style="left:' . $leftBarra . 'px; width:' . $larguraBarra . 'px; background-color:' . next($corHex) . ';" data-height="' . ($rs[$i]['qtd'] / $rsTotal) * 100 . '"><div>' . number_format(($rs[$i]['qtd'] / $rsTotal) * 100, 1, ',', '') . '%</div><div class="descricao">' . $rs[$i]['nome_motivo'] . '</div></div>';
		$leftBarra += ($larguraBarra + $espacoBarra);
	}
	?>
</div>
