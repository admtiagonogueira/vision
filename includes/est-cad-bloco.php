...
<?php getTitulo('CADASTRO DE BLOCO'); ?>
...
if (!empty($form_nome_bloco) && !empty($form_id_cidade)){
	...
	if ($numRows === 0){
		try{
			//Faz a inserção
			$rs = $conx->prepare('INSERT INTO fj_bloco VALUES(null, ?, ?, ?)');
			$rs->bindParam(1, $form_id_cidade);
			$rs->bindParam(2, $form_id_estado);
			$rs->bindParam(3, $form_nome_bloco);
			$rs->execute();
...
} else if (isset($_POST['submited']) && (empty($form_nome_bloco) || empty($form_id_cidade))) {
...
if (isset($_POST['form-del'])){
	try{
		$rs = $conx->prepare('DELETE FROM fj_bloco where id=?');
...
<form method="POST" action="/est-cad-bloco">
	<label>
	...
	<label>
		...
		<input type="text" size="50" maxlength="50" placeholder="Nome do Bloco" name="form-bloco" />
	</label>
	<br />
...
<?php getSubTitulo('Blocos cadastrados'); ?>

<?php
//Busca regiões cadastradas
try{
	$rs = $conx->prepare('
						 SELECT a.id, a.nome_bloco, b.nome_cidade, c.nome_estado FROM fj_bloco a
						 INNER JOIN fj_cidade b ON (a.fk_cidade_id = b.id)
						 INNER JOIN fj_estado c ON (a.fk_estado_id = c.id)
						 WHERE c.id = ? ORDER BY b.nome_cidade ASC , a.nome_bloco ASC
						');
	$rs->bindParam(1, $form_id_estado);
	$rs->execute();
} catch (PDOException $e) {
...
<?php while ($row = $rs->fetch(PDO::FETCH_ASSOC)) : ?>
		<tr>
			<td><?php echo $row['nome_bloco']; ?></td>
			<td><?php echo $row['nome_cidade']; ?></td>
			<td><?php echo $row['nome_estado']; ?></td>
			<td>
				<form action="/est-cad-bloco" method="POST">
