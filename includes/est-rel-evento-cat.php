...
try{
	$rs = $conx->prepare('
						 SELECT a.id, a.nome_evento, DATE_FORMAT(a.data_evento, \'%d/%m/%Y\') as data_evento, a.qtd, a.descricao, b.nome_igreja, d.nome_equipe  FROM fj_eventos a
						 INNER JOIN fj_q_igreja b ON (a.fk_q_igreja_id = b.id)
						 INNER JOIN fj_lider_equipe c ON (a.fk_lider_equipe_id = c.id)
						 INNER JOIN fj_equipe d ON (d.fk_lider_equipe_id = c.id)
						 WHERE a.fk_estado_id = ? AND b.catedral = \'S\'
