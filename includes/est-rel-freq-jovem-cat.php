...
<?php
//Busca as equipes da catedral de forma não repetida (DISTINCT)
try{
	...
	$rsIgreja->execute();
	$rsIgreja = $rsIgreja->fetchAll();
} catch (PDOException $e) {
	getDivResult(PAG_QUERY_ERR, DIV_ERR);
}

try{
	$rsEquipe = $conx->prepare('
								SELECT DISTINCT a.fk_equipe_id FROM fj_freq_equipe a
								INNER JOIN fj_equipe b ON (a.fk_equipe_id = b.id)
								WHERE a.fk_q_igreja_id = ?
								');
	$rsEquipe->bindParam(1, $rsIgreja[0]['fk_q_igreja_id']);
	$rsEquipe->execute();
} catch (PDOException $e) {
	getDivResult(PAG_QUERY_ERR, DIV_ERR);
}
...
try{
	$rs = $conx->prepare('
						 SELECT DATE_FORMAT(a.data_reuniao, \'%d/%m/%Y\') as data_reuniao, a.qtd_jovens, a.descricao, b.nome_equipe FROM fj_freq_equipe a
						 INNER JOIN fj_equipe b ON (a.fk_equipe_id = b.id)
						 WHERE a.fk_equipe_id = ?
						 ORDER BY a.data_reuniao DESC, b.nome_equipe ASC
						 LIMIT 0,2
						');
	$rs->bindParam(1, $row['fk_equipe_id']);
	$rs->execute();
...
if (!isset($rs[0]['nome_equipe'])) $rs[0]['nome_equipe'] = '';
...
?>
		...
		<tr>
			<td><?php echo $rs[0]['nome_equipe']; ?></td>
			<td>
