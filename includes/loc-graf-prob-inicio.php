...
<?php getTitulo('GRÁFICOS DE PROBLEMA (INICIAL) (' . date('d/m/Y', strtotime("-1 year")) . ' A ' .date('d/m/Y', strtotime("today")). ')'); ?>
...
try{
	$rs = $conx->prepare('
						...
						INNER JOIN fj_problemas b ON (a.fk_problema_inicial_id = b.id)
						WHERE a.entrada_iurd > DATE_ADD(NOW(), INTERVAL -1 YEAR)
						GROUP BY a.fk_problema_inicial_id
						');
	$rs->execute();
...
<p>
<span style="font-weight:bold;">OBS:</span> Este gráfico traz uma visão dos problemas quando os jovens chegam na igreja, no referido estado.<br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Para números mais detalhados, consulte a seção de Estatísticas.
</p><br />
